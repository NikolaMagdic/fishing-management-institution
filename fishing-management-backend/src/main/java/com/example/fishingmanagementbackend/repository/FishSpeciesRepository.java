...
public interface FishSpeciesRepository extends JpaRepository<FishSpecies, Long>{
    ...
    @Query(value = "SELECT DISTINCT * FROM fish_species fs LEFT OUTER JOIN inhabiting c ON fs.id = c.fish_species_id WHERE fishing_area_id = ?1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM fish_species fs WHERE fs.id NOT IN "
            + "(SELECT fish_species_id FROM fish_species fs LEFT OUTER JOIN inhabiting c ON fs.id = c.fish_species_id WHERE fishing_area_id = ?1)", nativeQuery = true)
