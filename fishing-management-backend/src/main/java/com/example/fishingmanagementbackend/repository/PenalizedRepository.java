...
public interface PenalizedRepository extends JpaRepository<Penalized, Long>{
    ...
    @Query(value = "SELECT p.id, p.date, p.report, p.penalty_id, p.fisherman_id FROM penalized p LEFT OUTER JOIN penalty pt ON p.penalty_id = pt.id WHERE p.fisherman_id = ?1", nativeQuery = true)
